...
namespace Demo.Mediator.DotNet4.Core
{
    ...
    public class UserRepository() : IUserRepository
    {
        ...
        public async Task<bool> CreateAsync(User user, CancellationToken cancellationToken = default)
        {
            ...
            Console.WriteLine($"Creating user with id {user.Id}");
            ...
            _users.Add(user);
            Console.WriteLine($"Created user {user.Id} with {user.Email}:");
            return true;
        }

        public async Task<User> UpdateAsync(User user, CancellationToken cancellationToken = default)
        {
            Console.WriteLine($"Updating user with {user.Id}", user.Id);
        ...
        {
            Console.WriteLine($"Getting user count with filter createdAfter {createdAfter}", createdAfter);
            // Simulate async operation
            await Task.Delay(100, cancellationToken);
        ...
        {
            Console.WriteLine($"Getting users with filter email {emailFilter} and buffer size {bufferSize}", emailFilter, bufferSize);
        
            // Simulate async operation
            await Task.Delay(100, cancellationToken);

            var filteredUsers = _users.Where(x => emailFilter == null || x.Email.Contains(emailFilter)).ToList();
