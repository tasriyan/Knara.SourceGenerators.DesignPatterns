...
namespace CodeGenerator.Patterns.Mediator
{
    ...
    private static string GenerateHandlerImplementation(HandlerInfo handler, List<RequestInfo> requests)
    {
        ...
        // Generate handler class that uses the NEW generated class name with FULLY QUALIFIED NAMES
        var requestFullName = !string.IsNullOrEmpty(request.Namespace) 
        ...
        // Generate Handle method with FULLY QUALIFIED TYPES
        var returnType = handler.Type switch
        {
            HandlerType.Query => $"System.Threading.Tasks.Task<{request.ResponseType ?? "object"}>",
            HandlerType.Command when !string.IsNullOrEmpty(request.ResponseType) => $"System.Threading.Tasks.Task<{request.ResponseType}>",
            HandlerType.StreamQuery => $"System.Collections.Generic.IAsyncEnumerable<{request.ResponseType ?? "object"}>",
            _ => "System.Threading.Tasks.Task"
        };
        ...
        if (handler.Type == HandlerType.StreamQuery)
        {
            sb.AppendLine($"    public async {returnType} Handle({requestFullName} {parameterName}, [EnumeratorCancellation] System.Threading.CancellationToken cancellationToken = default)");
        }
        else
        {
            sb.AppendLine($"    public async {returnType} Handle({requestFullName} {parameterName}, System.Threading.CancellationToken cancellationToken = default)");
        }
        sb.AppendLine("    {");

        // Map from the new generated class/record to the original request class/record
        var originalRequestFullName = !string.IsNullOrEmpty(request.Namespace) 
    ...
    private static string GenerateLegacyHandlerImplementation(LegacyMethodInfo method)
    {
        ...
        // Determine handler interface to implement with FULLY QUALIFIED NAMES
        var requestFullName = !string.IsNullOrEmpty(method.Namespace) 
            ? $"{method.Namespace}.{method.RequestName}"
            : method.RequestName;
        ...
        var returnType = method.HasReturnType 
            ? $"System.Threading.Tasks.Task<{method.ReturnType ?? "object"}>"
            : "System.Threading.Tasks.Task";

        sb.AppendLine($"    public async {returnType} Handle({requestFullName} request, System.Threading.CancellationToken cancellationToken = default)");
        sb.AppendLine("    {");
    ...
    private static string GenerateNotificationHandler(HandlerInfo handler)
    {
        ...
        sb.AppendLine("    }");
        sb.AppendLine();
        sb.AppendLine($"    public async System.Threading.Tasks.Task Handle({eventTypeFullName} eventObj, System.Threading.CancellationToken cancellationToken = default)");
        sb.AppendLine("    {");
        ...
        List<LegacyMethodInfo> legacyMethods)
    {
        ...
        sb.AppendLine();
        sb.AppendLine("    public GeneratedMediator(System.IServiceProvider serviceProvider)");
        sb.AppendLine("    {");
    ...
    private static void GenerateQuerySendMethod(StringBuilder sb, List<RequestInfo> requests, List<HandlerInfo> handlers)
    {
        ...
        foreach (var request in queryRequests)
        {
            ...
            if (handler != null)
            {
                ...
                sb.AppendLine($"            {requestFullName} typedQuery => (TResponse)(object)await ((Microsoft.Extensions.DependencyInjection.ServiceProviderServiceExtensions.GetRequiredService<{handlerFullName}>(_serviceProvider)).Handle(typedQuery, cancellationToken)),");
            }
        }

        sb.AppendLine("            _ => throw new System.InvalidOperationException($\"No handler registered for query type {query.GetType().Name}\")");
        sb.AppendLine("        };");
        sb.AppendLine("    }");
        sb.AppendLine();
    }
    ...
    private static void GenerateCommandSendMethod(StringBuilder sb, List<RequestInfo> requests, List<HandlerInfo> handlers)
    {
        ...
        foreach (var request in commandRequests)
        {
            ...
            if (handler != null)
            {
                ...
                sb.AppendLine($"            case {requestFullName} typedCommand:");
                sb.AppendLine($"                await ((Microsoft.Extensions.DependencyInjection.ServiceProviderServiceExtensions.GetRequiredService<{handlerFullName}>(_serviceProvider)).Handle(typedCommand, cancellationToken));");
                sb.AppendLine("                break;");
            }
        }

        sb.AppendLine("            default:");
        sb.AppendLine("                throw new System.InvalidOperationException($\"No handler registered for command type {command.GetType().Name}\");");
        sb.AppendLine("        }");
        sb.AppendLine("    }");
        sb.AppendLine();
    }
    ...
    private static void GenerateCommandWithResponseSendMethod(StringBuilder sb, List<RequestInfo> requests, List<HandlerInfo> handlers)
    {
        ...
        foreach (var request in commandRequests)
        {
            ...
            if (handler != null)
            {
                ...
                sb.AppendLine($"            {requestFullName} typedCommand => (TResponse)(object)await ((Microsoft.Extensions.DependencyInjection.ServiceProviderServiceExtensions.GetRequiredService<{handlerFullName}>(_serviceProvider)).Handle(typedCommand, cancellationToken)),");
            }
        }

        sb.AppendLine("            _ => throw new System.InvalidOperationException($\"No handler registered for command type {command.GetType().Name}\")");
        sb.AppendLine("        };");
        sb.AppendLine("    }");
        sb.AppendLine();
    }
    ...
    private static void GenerateLegacySendMethod(StringBuilder sb, List<LegacyMethodInfo> legacyMethods)
    {
        ...
        foreach (var method in voidMethods)
        {
            ...
            sb.AppendLine($"            case {requestFullName} typedRequest:");
            sb.AppendLine($"                await ((Microsoft.Extensions.DependencyInjection.ServiceProviderServiceExtensions.GetRequiredService<{handlerFullName}>(_serviceProvider)).Handle(typedRequest, cancellationToken));");
            sb.AppendLine("                break;");
        }

        sb.AppendLine("            default:");
        sb.AppendLine("                throw new System.InvalidOperationException($\"No handler registered for request type {request.GetType().Name}\");");
        sb.AppendLine("        }");
        sb.AppendLine("    }");
        sb.AppendLine();
    }
    ...
    private static void GenerateLegacySendWithResponseMethod(StringBuilder sb, List<LegacyMethodInfo> legacyMethods)
    {
        ...
        foreach (var method in responseMethods)
        {
            ...
            sb.AppendLine($"            {requestFullName} typedRequest => (TResponse)(object)await ((Microsoft.Extensions.DependencyInjection.ServiceProviderServiceExtensions.GetRequiredService<{handlerFullName}>(_serviceProvider)).Handle(typedRequest, cancellationToken)),");
        }

        sb.AppendLine("            _ => throw new System.InvalidOperationException($\"No handler registered for request type {request.GetType().Name}\")");
        sb.AppendLine("        };");
        sb.AppendLine("    }");
        sb.AppendLine();
    }
    ...
    {
        sb.AppendLine("    public async System.Threading.Tasks.Task Publish<TEvent>(TEvent eventObj, System.Threading.CancellationToken cancellationToken = default)");
        sb.AppendLine("    {");
        sb.AppendLine("        var eventType = typeof(TEvent);");
        sb.AppendLine("        var tasks = new System.Collections.Generic.List<System.Threading.Tasks.Task>();");
        sb.AppendLine();
        ...
        foreach (var handler in notificationHandlers)
        {
            ...
            sb.AppendLine("        {");
            sb.AppendLine($"            tasks.Add(((Microsoft.Extensions.DependencyInjection.ServiceProviderServiceExtensions.GetRequiredService<{handlerFullName}>(_serviceProvider)).Handle(({eventTypeFullName})(object)eventObj!, cancellationToken)));");
            sb.AppendLine("        }");
            sb.AppendLine();
        }
        ...
        sb.AppendLine("        {");
        sb.AppendLine("            await System.Threading.Tasks.Task.WhenAll(tasks);");
        sb.AppendLine("        }");
        sb.AppendLine("    }");
    }
    ...
    private static void GenerateCreateStreamMethod(StringBuilder sb, List<RequestInfo> requests, List<HandlerInfo> handlers)
    {
        ...
        foreach (var request in streamRequests)
        {
            ...
            if (handler != null)
            {
                ...
                sb.AppendLine($"            {requestFullName} typedQuery => (System.Collections.Generic.IAsyncEnumerable<TResponse>)((Microsoft.Extensions.DependencyInjection.ServiceProviderServiceExtensions.GetRequiredService<{handlerFullName}>(_serviceProvider)).Handle(typedQuery, cancellationToken)),");
            }
        }

        sb.AppendLine("            _ => throw new System.InvalidOperationException($\"No handler registered for stream query type {query.GetType().Name}\")");
        sb.AppendLine("        };");
        sb.AppendLine("    }");
    }
    ...
    private static void GenerateFullFrameworkQuerySendMethod(StringBuilder sb, List<RequestInfo> requests, List<HandlerInfo> handlers)
    {
        ...
        foreach (var request in queryRequests)
        {
            ...
            if (handler != null)
            {
                ...
                var fieldName = $"_{handler.HandlerName.Substring(0, 1).ToLower()}{handler.HandlerName.Substring(1)}";
                sb.AppendLine($"            {requestFullName} typedQuery => (TResponse)(object)await {fieldName}.Handle(typedQuery, cancellationToken),");
            }
        }
    ...
    private static void GenerateFullFrameworkCommandWithResponseSendMethod(StringBuilder sb, List<RequestInfo> requests, List<HandlerInfo> handlers)
    {
        ...
        foreach (var request in commandRequests)
        {
            ...
            if (handler != null)
            {
                ...
                var fieldName = $"_{handler.HandlerName.Substring(0, 1).ToLower()}{handler.HandlerName.Substring(1)}";
                sb.AppendLine($"            {requestFullName} typedCommand => (TResponse)(object)await {fieldName}.Handle(typedCommand, cancellationToken),");
            }
        }
    ...
    private static void GenerateFullFrameworkLegacySendWithResponseMethod(StringBuilder sb, List<LegacyMethodInfo> legacyMethods)
    {
        ...
        foreach (var method in responseMethods)
        {
            ...
            var fieldName = $"_{method.HandlerName.Substring(0, 1).ToLower()}{method.HandlerName.Substring(1)}";
            sb.AppendLine($"            {requestFullName} typedRequest => (TResponse)(object)await {fieldName}.Handle(typedRequest, cancellationToken),");
        }
    ...
    private static void GenerateFullFrameworkCreateStreamMethod(StringBuilder sb, List<RequestInfo> requests, List<HandlerInfo> handlers)
    {
        ...
        foreach (var request in streamRequests)
        {
            ...
            if (handler != null)
            {
                ...
                var fieldName = $"_{handler.HandlerName.Substring(0, 1).ToLower()}{handler.HandlerName.Substring(1)}";
                sb.AppendLine($"            {requestFullName} typedQuery => (System.Collections.Generic.IAsyncEnumerable<TResponse>){fieldName}.Handle(typedQuery, cancellationToken),");
            }
        }
    ...
    private static string GenerateExtendedDIExtensions(List<HandlerInfo> handlers, List<LegacyMethodInfo> legacyMethods)
    {
        ...
        sb.AppendLine("{");
        sb.AppendLine("    public static Microsoft.Extensions.DependencyInjection.IServiceCollection AddDeclarativeMediator(this Microsoft.Extensions.DependencyInjection.IServiceCollection services)");
        sb.AppendLine("    {");
        ...
        foreach (var handler in handlers)
        {
            ...
            sb.AppendLine($"        Microsoft.Extensions.DependencyInjection.ServiceCollectionServiceExtensions.AddScoped<{serviceFullName}>(services);");
            sb.AppendLine($"        Microsoft.Extensions.DependencyInjection.ServiceCollectionServiceExtensions.AddScoped<{handlerFullName}>(services);");
        }
